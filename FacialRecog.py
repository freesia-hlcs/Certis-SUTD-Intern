...
class FacialRecog(object):
    ...
    def add_new(self, name, img):
        ...
        if len(faces) == 1:
            ...
            print(name + ' face added')
        else:
            print('Invalid. Please ensure that there is only one face in the picture.')
    ...
    def detect_faces(self, test_img):
        img = test_img.copy()
        gray = cv2.cvtColor(img, cv2.COLOR_BGR2GRAY)
        faces = self.face_cascade.detectMultiScale(gray, scaleFactor=1.05, minNeighbors=7)
        if len(faces) > 0:
            return faces, gray
        else:
            return None, None
    ...
    def box_face(self, test_img, label, confidence, face):
        ...
        cv2.putText(img, self.name_list[label] + ' %.2f' % confidence, (x, y), cv2.FONT_HERSHEY_PLAIN, 1.5, (0, 255, 0), 2)
        return img
